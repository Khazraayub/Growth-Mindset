...
st.title("📀Datasweeper Sterling Integrator By Khazra Ayub")
st.write("Transform your files between CSV and Excel formats with built-in data cleaning and visualization on Creating the project for quarter 3!!")
        ...
        if st.checkbox(f"Clean data for {file.name}"):
            ...
            with col1:
                if st.button(f"Remove duplicates from the file: {file.name}"):
                    ...
                    st.write("✅ Duplicates rmoved!")
            
            with col2:
                if st.button(f"Fill missing values for {file.name}"):
                    numeric_cols = df.select_dtypes(include=['number']).columns
                    df[numeric_cols] = df[numeric_cols].fillna(df[numeric_cols].mean())
                    st.write("✅ Missing values had been filled!!")
